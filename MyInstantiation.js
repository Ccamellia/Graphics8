...
//左边
var thetaLeft = 0;	//左边长方体旋转角度
var deltaLeft = 60;	//左边长方体每秒旋转角度
//右边
var thetaRight = 0;	//左边长方体旋转角度
var deltaRight = 70;	//左边长方体每秒旋转角度
//中间
var yScalel = 1;		//中间长方体y方向缩放因子
var scaleFactorl = 1.1;	//yScale缩放系数
//中间
var xScalel = 1;		//中间长方体y方向缩放因子


//用于实现视角调整控制的变量
var angleY = 0.0;		//绕y轴旋转的角度
var angleX = 0.0;		//绕x轴旋转的角度
var angleStep = 0.0;	//角度变化步长（3度）
...
window.onload = function main(){
	...
	/*设置WebGL相关属性*/
	// 设置视口，占满整个canvas
    gl.clearColor(255/255, 248/255, 234/255, 1.0); // 设置背景色为白色
...
function render() {
	...
	//左
	gl.uniform4f(u_StartColor, 0.5, 0.92, 0.45, 1);	
	gl.uniform4f(u_EndColor, 1, 0.93, 0.94, 1);	
		drawCube(mult(translate(-16,15,0),
				mult(rotateZ(thetaLeft),scale(2.5,2.6, 2))));

	
	gl.uniform4f(u_StartColor, 205/255, 231/255, 244/255, 1);	
	gl.uniform4f(u_EndColor, 1, 0.93, 0.94, 1);	
		drawCube(mult(translate(-16,5,0),
				mult(rotateZ(thetaLeft),scale(2.5,2.6, 2))));

	
	
	gl.uniform4f(u_StartColor, 209/255, 186/255, 202/255, 1);	
	gl.uniform4f(u_EndColor, 1, 0.93, 0.94, 1);	
		drawCube(mult(translate(-16,-5,0),
				mult(rotateZ(thetaLeft),scale(2.5,2.6, 2))));


	gl.uniform4f(u_StartColor, 1, 0.63, 0.74, 1);	
	gl.uniform4f(u_EndColor, 1, 0.93, 0.94, 1);
		drawCube(mult(translate(-16,-15,0),
				mult(rotateZ(thetaLeft),scale(2.5,2.6, 2))));
	...
	//zhongjian 
	gl.uniform4f(u_StartColor, 236/255,245/255,244/255, 1);	
	gl.uniform4f(u_EndColor, 0.94, 0.82, 0.97, 1);	
	for(var j=1;j<360;j+=0.5)
	{
		drawCube(mult(translate(-5, 5,-5), mult(translate(6,-(yScale-10.5),0),
				mult(rotateZ(10*j),scale(4.5,4.6, 2)))));
	}
	
	
	gl.uniform4f(u_StartColor, 0.94, 0.82, 0.97, 1);	//中心为黑色
	gl.uniform4f(u_EndColor, 0.9, 0.44, 0.62, 1);	//最外围为绿色
	drawCube(mult(mult(rotateY(thetaRight),translate(0,-10,0)),
					mult(rotateX(90),scale(yScale,yScale,2))));	//绘制立方体
